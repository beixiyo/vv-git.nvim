...
- **冲突窗口按键死循环**：Result 窗口开着 `cursorbind`，从 ours / theirs 窗口按 `<` `>` `=` 时 Result 光标每次都被重置到块外，"先跳转再接受"永远到不了第二步。现在只有在 Result 窗口按键才要求光标在块内
...
- **不可逆操作确认**：discard 与 worktree 删除改用结构化确认和事务预检；执行前校验当前仓库及 worktree 的路径、分支、HEAD、锁定和 dirty 状态，普通删除被 Git 拒绝后才允许确认强制删除
- **命令执行上下文**：`X` 执行文件时使用解析计划提供的工作目录，并在面板关闭、切根或确认被新请求取代后取消待执行命令

### Fixed

- **过期回调隔离**：为仓库上下文增加 generation 校验，关闭、切换仓库或 A→B→A 后的旧 stage / unstage / discard / commit / push / execute 回调不再写入或刷新新会话
- **请求隔离**：关闭面板、切换仓库或发起新请求后，旧的 discard、worktree 删除与执行确认不再产生副作用

## 0.3.10 - 2026-08-07

### Added

- **目录预览**：光标停在目录行时，右侧显示该子树的变更文件数与状态分布
- **配置**：新增 `directory_preview`，默认开启，依赖 `preview`

### Fixed

- **目录视图边界**：目录预览不再改写「当前文件」，`gf` 也不会把 `:edit` 指向一个目录

## 0.3.9 - 2026-08-06

### Changed

- **索引写入队列**：stage、unstage、commit 与冲突接受按真实 Git index 路径串行执行；共享 `GIT_INDEX_FILE` 与同仓库快速操作不再并发争抢 `index.lock`
- **快速连续暂存**：左侧面板和右侧 diff 中连续按 `-` 会在按键时捕获明确文件并推进目标，队列排空后统一刷新，不再重复操作刷新前的旧节点

### Fixed

- **索引锁安全**：写入前解析真实 index 路径并保留 Git 原生互斥；检测到活动或陈旧锁时明确拒绝且绝不自动删除锁文件
- **过期写入保护**：面板关闭、仓库切换或提交浮窗失效后，尚未启动的 index 写入不会继续修改旧仓库，也不会由迟到回调刷新新会话
- **连续操作落点**：快速 stage / unstage 的失败、刷新合并与右侧异步视图不再遗留错误 action hint、跳回旧文件或漏掉后续文件
...
- 监听 `User VVExplorerRootChanged`，面板跟随 vv-explorer 的 `]` / `[` 切根。面板开着时解析该目录的仓库根，与当前 `git_root` 不同才切换（同仓库内换目录不重载，避免白闪一次），切换时按与 worktree 切换相同的语义清掉选中 / 比较 / 折叠 / 右侧视图后重载索引；面板关着时记住该目录，作为下次 `open()` 无显式 `root` / `path` 时的首选候选根 —— 此前只认 `getcwd()`，在 explorer 里切了仓库再开面板仍是旧仓库。目录已被删或不在任何仓库里时回退原有的 `getcwd()` 语义
...
- 面板顶部的当前分支名改用主题 Keyword 色，与分支 icon 和仓库名形成清晰层级
